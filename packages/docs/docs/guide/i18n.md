...
* `titles` : List of all full localized titles for every CRUD actions. Valid child keys are `list`, `show`, `create` or `edit`.
...
| Page Titles | `resources.{resource}.titles`            | `resources.monsters.titles`             |
...
For localized number and date field resource values, you will probably use the [number field](components/fields#number-field) and the [date field](components/fields#date-field). This components will use specific Vue I18n functions under the hood :

* [`$n`](https://kazupon.github.io/vue-i18n/guide/number.html)
